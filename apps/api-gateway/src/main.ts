...
async function bootstrap(): Promise<void> {
  ...
  app.getHttpAdapter().getInstance().use(notificationSocketProxy);
  ...
  if (process.env.NODE_ENV !== 'production') {
    const swaggerConfig = new DocumentBuilder()
      ...
      .setDescription('Enterprise API Gateway for Payflow Microservices')
      .setVersion('1.0.0')
      .addBearerAuth(
        {
          type: 'http',
  ...
  attachNotificationSocketUpgrade(app.getHttpServer(), notificationSocketProxy);
